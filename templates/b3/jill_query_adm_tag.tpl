<div class="container-fluid">
    <form action="tag.php" method="post" id="myForm" class="form-horizontal" role="form">
        ...
        <tr>
            <th><{$smarty.const._MA_JILLQUERY_TAG_DEMO}></th>
            <th><{$smarty.const._MA_JILLQUERY_TAG_TITLE}></th>
            <th><{$smarty.const._MA_JILLQUERY_TAG_FONTCOLOR}></th>
            <th><{$smarty.const._MA_JILLQUERY_TAG_COLOR}></th>
            <th><{$smarty.const._MA_JILLQUERY_TAG_ENABLE}></th>
            <th><{$smarty.const._MA_JILLQUERY_TAG_FUNC}></th>
        </tr>

        <{if $tag_sn==""}>
            <tr class="info">
            <td><{$smarty.const._MA_JILLQUERY_TAG_NEW}></td>
            ...
            <td>
                ...
                <input type="radio" name="enable" value="1" checked><{$smarty.const._YES}>
                </label>
                <label class="radio-inline">
                <input type="radio" name="enable" value="0" ><{$smarty.const._NO}>
                </label>
            </td>
            <td>
                <input type="hidden" name="op" value="insert_jill_query_tags">
                <{$XOOPS_TOKEN}>
                <button class="btn btn-primary" type="submit"><{$smarty.const._TAD_SAVE}></button>
            </td>
            </tr>
        <{/if}>
        ...
        <{foreach item=tag from=$tagarr}>
            ...
            <tr class="warning">
                ...
                <td>
                <label class="radio-inline">
                    <input type="radio" name="enable" value="1"  <{if $tag.checked=='1' or $tag.checked==''}>checked<{/if}>><{$smarty.const._YES}>
                </label>
                <label class="radio-inline">
                    <input type="radio" name="enable" value="0" <{if $tag.checked=='0'}>checked<{/if}>><{$smarty.const._NO}>
                </label>
                </td>
                <td>
